...
const sqsColumnDefinitions: ColumnDefinition[] = [
  ...
  {
    accessor: 'RedrivePolicy',
    Header: 'Redrive Policy',
    Cell: ({ value }) => <DefaultCell value={value} />
  },
  {
...
function getRows(
  queueDetails: GetQueueAttributesCommandOutput
): SQSRowDefinitions[] {
  const rows: any[] = [];
  if (!queueDetails) {
    return rows;
  ...
  return [
    {
      ...
      ApproximateNumberOfMessages:
        queueDetails.Attributes['ApproximateNumberOfMessages'],
      ApproximateNumberOfMessagesDelayed:
        queueDetails.Attributes['ApproximateNumberOfMessagesDelayed'],
      ApproximateNumberOfMessagesNotVisible:
        queueDetails.Attributes['ApproximateNumberOfMessagesNotVisible'],
...
const emptySqsTableData: TableData = {
  columns: sqsColumnDefinitions,
  rows: []
};

function getTableData(
  getQueueAttributesCommandOutput: GetQueueAttributesCommandOutput
): TableData {
...
const emptyMessageTableData: TableData = {
  columns: sqsColumnDefinitions,
  rows: []
};
...
function DeleteMessageForm({
  ...
  const handleSubmit = (
    values: ValuesSchema,
    actions: FormikHelpers<ValuesSchema>
  ) => {
      ...
      >
        <Grid item xs={12} lg={8}>
          <Formik
            ...
            validationSchema={deleteMessageFormValidation}
                    ...
                    <Grid container spacing={3}>
                      <Grid item xs={12} sm={12}>
                        <FormField
                          ...
                          success={
                            values.receiptHandle.length > 0 &&
                            !errors.receiptHandle
                          }
                        />
                      </Grid>
      ...
      client
        ...
        .then((output) => {
          if (output && output.QueueUrls && output.QueueUrls.length > 0) {
            ...
            output.QueueUrls.forEach((queueUrl) =>
              newQueueMap.set(getQueueNameFromUrl(queueUrl), queueUrl)
            );
      ...
      client
        .send(
          new GetQueueAttributesCommand({
            QueueUrl: queueMap.get(queue),
            AttributeNames: attributesToFetch
          })
        )
          ...
          >
            ...
            <MDBox display="flex" justifyContent="space-between">
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <MDInput {...params} label="Queue" fullWidth />
                )}
              />
            </MDBox>
          </MDBox>
