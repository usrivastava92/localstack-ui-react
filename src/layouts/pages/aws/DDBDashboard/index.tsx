...
function getColumnDefinitions(
    ...
    Object.keys(item).forEach((key) => {
      if (!columnsDefsAlreadyAdded.has(key)) {
        columnDefinitions.push({
          ...
          accessor: key,
          Cell: (value) => (
            <DefaultCell value={getStringAttributeValue(value.value)} />
          )
        });
  ...
  return columnDefinitions.sort((one, two) =>
    one.Header < two.Header ? -1 : 1
  );
}
          ...
          >
            ...
            <MDBox display="flex" justifyContent="space-between">
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <MDInput {...params} label="Table" fullWidth />
                )}
              />
            </MDBox>
          </MDBox>
...
function DDBDashboard(): JSX.Element {
  return (
    <AwsDashboardLayout
      title="Dynamo DB"
      subTitle="Create/Choose a profile and then select a table to view your data"
    >
      <Content />
    </AwsDashboardLayout>
  );
